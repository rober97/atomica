		...
		var htmlObject = $(' \
			<tr class="title" data-categoria="0"> \
				<th style="min-width: 90px !important;"> \
					<span class="ui-icon ui-icon-arrow-4" title="Arrastrar para mover" style="cursor: move;"></span><button class="ui-icon ui-icon-minus remove categoria" title="Quitar categoría"></button> \
				...
				<th style="min-width: 150px;"><input name="item[][nombre]" type="search" value="" placeholder="Buscar categoría por nombre..."><button class="ui-icon ui-icon-plus add all-items"><!-- <button class="show categoria">Ver categorías</button> --><button class="ui-icon ui-icon-document detail categoria" title="Detalle">Detalle</button><button class="ui-icon ui-icon-gear profile categoria" title="Perfil">Perfil</button></th> \
				...
				<th class="costo"></th> \
				<th class="numeric currency costo presupuestado">' + localCurrency + ' <span><input readonly name="item[][subtotal_costo]" type="text" value="0"></span></th> \
				...
				<th class="aplica-sobrecargo costo-interno" colspan="2"></th> \
				<th class="ocultar-print"><input name="item[][ocultar_print]" type="checkbox" value="true"></th> \
				<th><input name="item[][selected]" type="checkbox" value="true"></th> \
			</tr> \
		');
	...
	item: function(functor, element) {
		var htmlObject = $(' \
			<tr data-producto="0"> \
				...
				<td class="numeric code"><input name="item[][codigo]" type="text" ' + ((editCodigoItems)? '' : ' readonly') + '></td> \
				...
				<td class="tipo-documento"><input type="text" disabled name="item[][tipo_documento]"><button class="ui-icon ui-icon-carat-1-s show tipo-documento">Ver tipos de documento</button></td> \
				...
				<td class="numeric currency costo presupuestado">' + localCurrency + ' <span><input name="item[][costo_unitario]" type="text" text="0" max="9999999999" value="0"></span></td> \
				<td class="numeric currency costo presupuestado">' + localCurrency + ' <span><input' + ((subtotal_gasto_p_manual)? '' : ' readonly' ) + ' name="item[][subtotal_costo]" type="text" value="0"></span> <button class="ui-icon ui-icon-notice detail cost" title="Ver detalle del costo">Ver detalle del costo</button></td> \
				...
				<td class="numeric currency costo real adquisicion">' + localCurrency + ' <span><input readonly name="item[][subtotal_costo_real]" value="0"></span> <button class="ui-icon ui-icon-notice detail cost-real" title="Ver detalle del costo">Ver detalle del costo</button> <span name="item[][closed_compras]" class="ui-icon ui-icon-locked" style="display: inline-block;"></span></td> \
				...
				<td class="fit ocultar-print"><input readonly name="item[][ocultar_print]" type="checkbox" value="true"></td> \
...
var updateIndexes = function(callback) {
	...
	$('#tabs-2a section.sheet table > tbody > tr').each(function(key, item) {
		...
		if ($(item).hasClass('title')) {
			...
			fields['item[' + k + '][closed_compras]'] = $(item).find('span[name="item[][closed_compras]"]').is(':visible');
...
var updateTotalUtilidadCosto = function() {
	...
	costo_real_otros = summaryNegocioData.rows[0].resumen.gasto_real.otros.value / exchange_rate;
	costo_real_sobrecargos = summaryNegocioData.rows[0].resumen.gasto_real.sobrecargos.value / exchange_rate;
	...
	if (typeof selected_currency == 'undefined') {
		...
		var resultado_evento = (delta - gasto_of).toFixed(currency.decimals).toString().replace('.', ',');
		...
		var total_comision_ejecutivo = (ca * ejecutivo_ca_ratio_raw + resultado_evento * ejecutivo_resultado_ratio_raw).toFixed(currency.decimals).toString().replace('.', ',');
		...
		var resultado_evento = (delta - gasto_of).toFixed(2).toString().replace('.', ',');
		...
		var total_comision_ejecutivo = (ca * ejecutivo_ca_ratio_raw + resultado_evento * ejecutivo_resultado_ratio_raw).toFixed(2).toString().replace('.', ',');
...
var updateSubtotalNeto = function() {
	...
	if (typeof selected_currency == 'undefined') {
		$('span[name="cotizacion[montos][subtotal_neto]"]').text(Math.abs((subtotal_precios + subtotal_sobrecargos - descuento).toFixed(currency.decimals)).toString().replace('.', ','));
...
var updateSubtotalItems = function() {
	...
	target.find('tr').not('.title').each(function() {
		...
		subtotal_diferencias+= parseFloat($(this).find('[name="item[][diferencia]"]').val());
...
var updateRow = function(event) {
	...
	if (subtotal_gasto_p_manual && $(event.target).prop('name') == 'item[][subtotal_costo]') {
		var costo_unitario = parseFloat(target.find('[name="item[][subtotal_costo]"]').val()) / (parseFloat(target.find('[name="item[][cantidad]"]').val()) * parseFloat(target.find('[name="item[][factor]"]').val()));
	...
	if (target.data('first-load') !== true) {
		updateSubtotalTitulos($(event.target));
		updateSubtotalItems();
	}
...
var getDetail = function(callback) {
	$.ajax({
		url: '/4DACTION/_V3_getItemByPresupuesto',
		dataType: 'json',
		data: {
		...
		success: function(data) {
			...
			for (var i = 0; i < data.rows.length; i++) {
				...
				if (item.titulo) {
					...
					const diff = (item.costo.presupuestado.subtotal - item.costo.real.subtotal) / exchange_rate

					if(diff < 0){
						htmlObject.find('[name="item[][diferencia]"]')[0].style.color = 'red'
					}
			...
			if ($('section.sheet').data('readonly') || $('section.sheet').data('locked')) {
				...
				$('section.sheet').find('table.items tbody tr').draggable('destroy');
			}
