...
$(document).ready(function() {
		...
		console.warn('editor.js crom script/startup');
	...
	$('section.sheet table').on('click', 'tbody tr.title button.toggle.categoria', function() {
		//TODO verificar calcular sin hacer trigger del click, entra por cada categoria para calcular totales
			console.log('script/startup/editor')
...
sobrecargos.getSobrecargosInfo();
	...
	var updatePrecioCotizado = function(event) {
		...
		if ($(this).parentTo('tr').data('tipo-documento') && ($(this).parentTo('tr').data('tipo-documento-ratio') || (!$(this).parentTo('tr').data('tipo-documento-ratio') && $(this).parentTo('tr').data('tipo-documento-valor-usd'))) ) {
			...
			var valor_usd = $(this).parentTo('tr').data('tipo-documento-valor-usd'); // Impuesto extranjero
			...
			if (typeof selected_currency == 'undefined') {
				if (division) {
					// Impuesto extranjero
				   if (valor_usd)
					   base_imponible = (valor_negociado / (1.00 - impuesto)) * valor_usd_cotizacion;
				   else
	...
	var updatePrecioNegociado = function(event) {
		...
		if ($(this).parentTo('tr').data('tipo-documento') && ($(this).parentTo('tr').data('tipo-documento-ratio') || (!$(this).parentTo('tr').data('tipo-documento-ratio') && $(this).parentTo('tr').data('tipo-documento-valor-usd'))) ) {
			...
			if (multiplicacion) {
				// Impuesto extranjero
				if (valor_usd)
					valor_cotizado = (valor_a_cotizar * (1 - impuesto)) / valor_usd_cotizacion;
				else
	...
	$('section.sheet').on('hover', 'button.detail.price', function(event) {
		...
		$(this).tooltipster({
			content: function() {
				...
				// Impuesto extranjero
				if (valor_usd)
					htmlObject.find('li[data-name="valor-negociado"] span.valor-usd').text(' (USD ' + (valor_negociado / valor_usd_cotizacion).toFixed(2).replace(/\./g, ',') + ')'); // Impuesto extranjero
	...
	$('section.sheet').on('hover', 'button.detail.expenses-details', function(event) {
						...
						content: function() {
							...
							$.each(data.rows, function(key, item) {
								 ...
								 htmlObject += `<br><br>
												        ...
												        <span style="margin: 100px;" class="ui-selectable"> ${item.folio}</td>
												        <br><br>
												         <span style="margin: 100px;" class="ui-selectable"> ${item.folio}</td>
												        <br><br>
												         <span style="margin: 100px;"> ${item.folio}</td>
												        <br><br>
												         <span style="margin: 100px;"> ${item.folio}</td>
												        <br><br>
												         <span style="margin: 100px;"> ${item.folio}</td>
...
*/
	...
	(function() {
		...
		var htmlObject = $('\
				<ul class="dropdown-menu" style="position: absolute; top: 25px; right: 0; z-index: 1000; min-width: 150px; text-align: left;">	\
					...
					-->																															\
					<li><a href="#" class="definir-sobrecargo items"><span class="ui-icon ui-icon-radio-on"></span>Definir sobrecargo</a></li>  \
				</ul>																															\
			');
	...
	$('button.profile.empresa').click(function() {
		console.log('ID EMPRESA:', $('.input-empresa').data('id'));
		console.log('ALIAS EMPRESA:', $('.input-empresa').val());
