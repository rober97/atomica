...
unaBase.toolbox.menu.newInit = function (params) {
...
unaBase.toolbox.menu.init = function (params) {
      ...
      action: function () {
        if (comprobantes.id > 0) {
          ...
          let html = $(`<tr data-id="0">
                              ...
                              <td class="numeric currency"><input  name="debe" value="0" type="text" /></td>
                              <td class="numeric currency"><input  name="haber" value="0" type="text" /></td>
  ...
  if ($.inArray("accounting_libro_diario", params.buttons) != -1) {
    ...
    buttons.push({
      ...
      action: function () {
        ...
        let url = nodeUrl + "/libro-diario/?download=true&sid=" + unaBase.sid.encoded() + "&" + filters + "&hostname=" + window.location.origin;
  ...
  if ($.inArray("accounting_libro_mayor", params.buttons) != -1) {
    buttons.push({
      ...
      action: function () {
        ...
        let url = nodeUrl + "/libro-mayor/?download=true&sid=" + unaBase.sid.encoded() + "&" + filters + "&hostname=" + window.location.origin;
        ...
        action: function() {
            ...
            prompt(htmlObject).done(function(data) {
                ...
                if (fecha_periodo_to !== "" && fecha_periodo_from !== "") {
                    ...
                    let url = nodeUrl + "/balance/?download=true&dateto=" + fecha_periodo_to + "&datefrom=" + fecha_periodo_from + "&sid=" + unaBase.sid.encoded() + "&" + filters + "&hostname=" + window.location.origin;
    ...
    buttons.push({
      ...
      action: function (event) {
        ...
        if (
          ...
          unaBase.ui.block();
          // saveAction();
          setTimeout(generalSaveAction(elementCobro,eventData,event), 1000);
        } else {
          generalSaveAction(elementCobro,eventData,event);
        }
  ...
  if ($.inArray("dtv_charge_selection", params.buttons) != -1)
    buttons.push({
      ...
      action: function () {
        var selected = $("table. tbody").find("input:checked");
  ...
  */
  ...
  if ($.inArray("preview", params.buttons) != -1)
    buttons.push({
      ...
      action: function () {
        ...
        unaBase.save.callback = callback;
        var saved = menu
          .find('[data-name="save"] button')
          .triggerHandler("click");
        setTimeout(() => {
  ...
  if ($.inArray("import_gastos", params.buttons) != -1)
    buttons.push({
      ...
      caption: `Importar y crear ${$("body nav ul li.active").data("name")=='gastos'?'OC':'FXR'} desde excel`,
      action: function () {
        let tipo = "";
        if (
  ...
  if ($.inArray("importJustificarMasivo", params.buttons) != -1)
    buttons.push({
      ...
      icon: "ui-icon-calculator",
      caption: `Importar y justificar ${$("body nav ul li.active").data("name")=='gastos'?'OC':'FXR'} masivo`,
      action: function () {
        let tipo = "";
        ...
        $(this).tooltipster({
          content: function () {
            ...
            let saveAction = async callback => {
              ...
              var saved = await generalSaveAction(elementCobro,eventData)
  ...
  if ($.inArray("share_oc_val", params.buttons) != -1) {
    buttons.push({
      ...
      action: function () {
        ...
        var sendOcByEmail = function (address) {
          unaBase.ui.block();
  ...
  if ($.inArray("open_accounting_period", params.buttons) != -1)
    buttons.push({
      ...
      action: function () {
        ...
        let abrirPeriodo = function (period, action) {
  ...
  if ($.inArray("close_accounting_period", params.buttons) != -1)
    buttons.push({
      ...
      action: function () {
        ...
        let cerrarPeriodo = function (period, action) {
  ...
  if ($.inArray("create_accounting_period", params.buttons) != -1)
    buttons.push({
      ...
      action: function () {
        ...
        let crearPeriodo = function (period) {
    ...
    buttons.push({
      ...
      action: function () {
        ...
        $("#viewport tbody.ui-selectable tr[data-id] td:first-of-type input.selected:checked").each(function () {
          ...
          if ($(this).closest("tr").data("forma-pago") != "TRANSFERENCIA" && $(this).closest("tr").data("forma-pago") != "VALE VISTA" && $(this).closest("tr").data("forma-pago") != "") {
        ...
        switch (modulo) {
          case "EGRESOS":
            ...
            axios("/4DACTION/_V3_getParamEgreso")
              .then(resp => {
                if (!pagada && emitida && !sin_doc_pago && !excluir_tipo_pago) {
                  var htmlObject = $(
                    '<section> \
                  ...
                  var promptNomina = function () {
                    prompt(htmlObject).done(function (data) {
                      if (data !== false && data !== "") {
                        var exportSoftlandNomina = function (filename, nominaType = "bciGrand") {
  ...
  if ($.inArray("oc_por_validar", params.buttons) != -1)
    buttons.push({
      ...
      action: function () {
        ...
        let url = `${nodeUrl}/expenses/tovalidate?sid=${unaBase.sid.encoded()}`;
        console.log(url);
...
const generalSaveAction = async (elementCobro,eventData,event) => {
        ...
        if (data.success) {
          ...
          switch (params.entity) {
            ...
            case "Negocios":
              ...
              var result =await  updateIndexes(callback);
            ...
            case "Dtc":
              ...
              const saveDtc = () => {
                ...
                return new Promise((resolve, reject) => {
                  ...
                  console.log("datos\n\n:" + dtc.container.serializeAnything());
                  $.ajax({
                ...
                saveDocIngreso()
                  .then(res => {
                    ...
                    $(".ui-dialog")
                      ...
                      .trigger("click");
                    unaBase.loadInto.dialog(
                      '/v3/views/dtc/dialog/confirmacion_Comprobante.shtml?id=' + ingresoId + '&tipoDocRef=INGRESO',
                      "FAVOR CONFIRMAR CUENTA CONTABLE PARA CONTINUAR",
                      "x-large",
                      false,
                      false,
