...
$(document).ready(function () {
	...
	$('button.edit.save.contacto').click(function (event) {
		...
		const fk = Number($('input[name="cotizacion[empresa][id]"]').data('id') || 0);
		const id = Number($('input[name="cotizacion[empresa][contacto][id]"]').data('id') || 0);

		// OBLIGATORIO
		const nombre = String($('input[name="cotizacion[empresa][contacto][id]"]').val() || '').trim();
		if (!nombre) {
			toastr.error('El nombre del contacto es obligatorio.');
			$('input[name="cotizacion[empresa][contacto][id]"]').focus();
			return;
		}

		const cargo = String($('input[name="cotizacion[empresa][contacto][cargo]"]').val() || '').trim();
		const email = String($('input[name="cotizacion[empresa][contacto][email]"]').val() || '')
			.trim()
			.toLowerCase();

		if (!fk) {
			toastr.error('Empresa inválida (fk vacío).');
			return;
		}
		...
		const fields = {
			...
			"cotizacion[empresa][contacto][cargo]": cargo,
			"cotizacion[empresa][contacto][email]": email
		};
		...
		$.ajax({
			...
			success: function (data) {
				if (data.success) {
					toastr.info(data.new ? 'Contacto creado!' : 'Contacto modificado!');

					$('input[name="cotizacion[empresa][contacto][id]"]')
						.data('id', data.id)
						.attr('data-id', data.id);

					$('h2 [name="cotizacion[empresa][contacto][id]"]').text(nombre);

					afterEditContacto(element);
				} else {
					if (data.opened) {
						if (data.readonly) toastr.error('No fue posible guardar. Otro usuario está bloqueando el registro.');
						else toastr.error('No fue posible guardar (error desconocido).');
					} else {
						if (data.unique === false) toastr.error('El contacto ya existe (duplicado).');
						else toastr.error('El id del contacto no existe o no se pudo abrir.');
					}
				}
			},
			error: function () {
	...
	unaBase.toolbox.form.autocomplete({
		...
		search: function () {
			$(this).data('hasResults', null);   // desconocido hasta response
		},

		response: function (event, ui) {
			const $input = $(this);
			const target = $input.parentTo('ul');

			$input.data('hasResults', Array.isArray(ui?.content) && ui.content.length > 0);
			$input.data('id', null);
		...
		change: function (event, ui) {
			...
			const hasId = !!$input.data('id');
			const hasResults = $input.data('hasResults') === true;
			const noResults = $input.data('hasResults') === false;

			// ✅ NUEVO: hubo resultados pero no se seleccionó nada → vaciar y enfocar (obliga a seleccionar)
			if (!hasId && hasResults) {
				$input.val('');
				// opcional: limpiar también “forma de pago” y otros campos dependientes si corresponde
				setTimeout(() => $input.focus(), 0);
				return;
			}

			// ❗ Solo preguntar para crear cuando NO hubo resultados
			if (!hasId && noResults) {
				const element = this;

				if (!access._551) {
					confirm('El cliente "' + value + '" no está registrado.' + "\n\n" + '¿Desea crearlo?')
						.done(function (ok) {
							if (ok) {
		...
		select: function (event, ui) {
			...
			if (typeof v3_sobrecargos_cinemagica !== 'undefined' && v3_sobrecargos_cinemagica && ui.item.porcentaje_sobrecargo_ca > 0) {
			...
			$.ajax({
				...
				dataType: 'json',
				async: false,
				data: { id: ui.item.id, default: true, strict: true },
				success: function (data) {
					const $ct = $('input[type="search"][name="cotizacion[empresa][contacto][id]"]').parentTo('ul');
					$ct.find('input[name="cotizacion[empresa][contacto][id]"]').attr('data-id', 0).val('');
					$ct.find('input[name="cotizacion[empresa][contacto][cargo]"]').val('');
					$ct.find('input[name="cotizacion[empresa][contacto][email]"]').val('');
					$('h2 [name="cotizacion[empresa][contacto][id]"]').text('');

					$.map(data.rows, function (item) {
						$ct.find('input[name="cotizacion[empresa][contacto][id]"]').attr('data-id', item.id).val(item.nombre_completo);
						$ct.find('input[name="cotizacion[empresa][contacto][cargo]"]').val(item.cargo);
						$ct.find('input[name="cotizacion[empresa][contacto][email]"]').val(item.email);
						$('h2 [name="cotizacion[empresa][contacto][id]"]').text(item.nombre_completo);
					});

					$ct.find('button.unlock.contacto, button.profile.contacto, button.show.contacto').show();
					$ct.find('button.edit.contacto').hide();
				}
			});
	...
	unaBase.toolbox.form.autocomplete({
		fields: [
			...
			{ local: 'cotizacion[empresa][contacto][email]', remote: 'razon_social', type: 'email' } // <- si tu API devuelve 'email', cambia remote a 'email'
		],
		data: {
			entity: 'Contacto',
		...
		change: function (event, ui) {
			...
			var hasId = !!$input.data('id');
			var hasResults = $input.data('hasResults') === true;
			var noResults = $input.data('hasResults') === false;

			// Hubo resultados pero no se seleccionó ninguno -> obligar a seleccionar
			if (!hasId && hasResults) {
				$input.val('');
			...
			// No hubo resultados -> confirmar creación
			if (!hasId && noResults) {
				var element = this;

				if (!access._551) {
					confirm('El contacto "' + value + '" no está registrado.' + "\n\n" + '¿Desea crearlo?')
						.done(function (ok) {
							if (ok) {
								$(element).data('id', null);
								$('button.unlock.contacto').click();
		...
		select: function (event, ui) {
			var $input = $('input[type="search"][name="cotizacion[empresa][contacto][id]"]');
			var target = $input.parentTo('ul');
			...
			// Ajuste de empresa si el contacto pertenece a otra empresa
			...
			if (!old_empresa) {
				...
				$.ajax({
					...
					dataType: 'json',
					async: false,
					data: { q: new_empresa, filter: 'id' },
					success: function (data) {
						var targetE = $('input[type="search"][name="cotizacion[empresa][id]"]').parentTo('ul');

						$.map(data.rows, function (item) {
							targetE.find('input[name="cotizacion[empresa][id]"]').data('id', item.id);
							targetE.find('input[name="cotizacion[empresa][id]"]').val(item.text);
							targetE.find('input[name="cotizacion[empresa][razon_social]"]').val(item.razon_social);
							targetE.find('input[name="cotizacion[empresa][rut]"]').val(item.rut);
							targetE.find('input[name="cotizacion[empresa][rut]"]').data('rut', item.rut);

							targetE.find('input[name="cotizacion[empresa][id]"]').data('sobrecargo-ca', item.porcentaje_sobrecargo_ca);
