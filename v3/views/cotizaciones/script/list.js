...
const createTableItem = (item) => {
  ...
  addTextCol(item.codigo);
  addTextCol(item.created_at, "center", "date");
  addTextCol(item.updated_at, "center", "date");
  addTextCol(item.fecha_realizacion, "center", "date");
  addTextCol(item.ejecutivo_nombre);
  addTextCol(item.block_user_name, "left");
  addTextCol(item.referencia_proyecto, "left");

  // Columna para el total neto
  const colNetTotal = document.createElement("div");
  colNetTotal.classList.add("col", "between");
  const spanSymbol = document.createElement("span");
  spanSymbol.textContent = item.exchange.type;
  const spanAmount = document.createElement("span");
  spanAmount.textContent = formatCurrencyList(item.costo.presupuestado);
  colNetTotal.appendChild(spanSymbol);
  colNetTotal.appendChild(spanAmount);
  tableItem.appendChild(colNetTotal);

  // Columna para validar
  addTextCol(item.validated ? "Si" : "No", "center", "status");
  ...
  var params = {
    ...
    init: (updateListCallback) => {
      ...
      initializeDateFilters([
        { id: 'created_at', defaultDate: 'Mayo 2024', defaultYear: 2024, defaultMonth: 5 }
      ], updateListCallback);
      // Inicializar botones
    ...
    row: {
      html: function (i, rows) {
        const item = {
          ...
          fecha_realizacion: rows[i].fecha_realizacion,
          ejecutivo_nombre: rows[i].ejecutivo || 'N/A',
          block_user_name: rows[i].block_user_name,
          referencia_proyecto: rows[i].text_proyecto,
          exchange: {
            type: rows[i].exchange ? rows[i].exchange.type : 'USD',
          },
          costo: {
            presupuestado: rows[i].total_neto
          },
    ...
    filters: [
      ...
      { name: 'por_emisor', type: 'ub-select', caption: 'Por emisor', dataSource: 'getUsuariosEmisores' },
      { name: 'ejecutivo_responsable', type: 'new_select', caption: 'Ejecutivo responsable', dataSource: 'getUsuarioEjecutivo?state=cot' },
      {
        name: 'validated', type: 'new_select', caption: 'Según validación', dataObject: [
          { id: 'true', text: 'Validadas' },
          { id: 'false', text: 'No validadas' }
        ]
      },
      {
        name: 'oc_proveedor', type: 'new_select', caption: 'Según OC Proveedor', dataObject: [
      ...
      {
        name: 'mas_busquedas', type: 'ub-select', caption: 'Más filtros', dataObject: [
      ...
      { name: 'por_proyecto', type: 'ub-select', caption: 'Por proyecto', dataSource: 'getProyectosActivos' },
      { name: 'personalizadoUno', type: 'ub-select', caption: 'campo1', dataSource: 'getParamForSearch1' },
      { name: 'personalizadoDos', type: 'ub-select', caption: 'campo2', dataSource: 'getParamForSearch2' },
      { name: 'personalizadoTres', type: 'ub-select', caption: 'campo3', dataSource: 'getParamForSearch3' },
      { name: 'motivo_anulacion', type: 'ub-select', caption: 'Motivo anulación', dataSource: 'getMotivoAnulacionFiltro?modulo=Cotizacion' }
    ],
    renderTableItems: (content) => {
