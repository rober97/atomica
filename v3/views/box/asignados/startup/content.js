...
$(document).ready(function() {
          	...
          	success: function(data) {          	
	            $.each(data.rows, function(key, item){
	            	if (item.venta_tarifa > 0) {
	            		var target = $('section.sheet > fieldset > table.items.ventas > tbody').find('button.add.item');
						...
						getElement.item('insertAfter', current, tipo, item);
	            	}
	            	if (item.compra_tarifa > 0) {
	            		var target = $('section.sheet > fieldset > table.items.compras > tbody').find('button.add.item');
		            	var current = target.parentTo('tr');
						var tipo = target.data('tipo');
						while(!(current.next().html() == undefined || current.next().find('th').length > 0)) {
							current = current.next();
		...
		$('section.sheet > fieldset > table > tbody').find('button.add.item').click(function(){
			var current = $(this).parentTo('tr');
			var tipo = $(this).data('tipo');
			while(!(current.next().html() == undefined || current.next().find('th').length > 0)) {
				current = current.next();
			}	
			getElement.item('insertAfter', current, tipo);
		});

		$('section.sheet > fieldset > table > tbody').on('click', 'tr:not(.title) button.remove.item', function() {
			var element = this;
