...
$(document).ready(function() {
		...
		console.warn('detalle.js from script/startup');
	...
	*/
	...
	$('section.sheet table > tbody').on('click', 'button.detail.categoria', function(event) {
		unaBase.loadInto.dialog('/v3/views/cotizaciones/pop_detalle_items.shtml?id=' + id, 'Detalle de Categoría', 'medium');
	});
		...
		$.each(data.rows, function(key, item) {
			...
			htmlObject.find('input[name="sobrecargo[' + item.id + '][real]"]').change(function(event) {
				...
				if (margenCompraResumenes) {
				    ...
				    utilidad_real_ratio = utilidad_real_total / total_neto * 100;
				    costo_real_ratio = costo_real_total / total_neto * 100;
				}
				if (!isFinite(utilidad_real_ratio)) utilidad_real_ratio = 0;
				if (!isFinite(costo_real_ratio)) costo_real_ratio = 0;
				$('span[name="cotizacion[montos][costo_real_ratio]"]').text((utilidad_real_ratio).toFixed(2)).number(true, 2, ',', '.');
				$('span[name="cotizacion[montos][utilidad_real_ratio]"]').text(((costo_real_ratio) / total_neto).toFixed(2)).number(true, 2, ',', '.');
