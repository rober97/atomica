...
const datatable = new DataTable('neg');
const dtitems = new DataTable('item');
	...
	if (cont == 0) {
		...
		bts.forEach(bts_ele => {
			...
			if (bts_ele.classList.contains(colorCheck)) {
				...
				if (bts_ele.classList.contains('yellow') && cont > 1)
					bts_ele.style.display = "none"
			}
	...
	if (red_type) {
		if (event.currentTarget.classList.contains('two')) {
			...
			ids.forEach(ids_e => {
				...
				for (i = 0; i <= 4; i++) {
					button1.push(`<button class="btn step-back" onclick="nextBack(this)" type="button" step="${i + 1}" idstep="${ids_e.id}" style="float: left; height: 2em; width: 5em; background-color: rgb(252, 73, 3); color: white; cursor: pointer; font-weight: bold;">Volver</button>`);
					button2.push(`<button class="btn step step-${i + 1}" onclick="nextBack(this)" type="button" step="${i + 1}" idstep="${ids_e.id}" style="float: right; height: 2em; width: 6em; background-color: #34CC02; color: white; cursor: pointer; font-weight: bold;">Siguiente</button>`);
				}
			...
			if (res.records.total_records > 0) {
				res.data.rows.forEach(function (res_ele) {
					...
					if (res_ele.tipo.includes("BOLETA"))
						...
						btn_pdf = `<button type="button" class="btn" title="Descargar pdf" onclick="downloadPDF(this)" id="${res_ele.id}" folio="${res_ele.folio}" style="display: "><i class="far fa-file-pdf fa-2x" style="color:#FC4903"></i></button>`;
...
const nextBack = (obj) => { 
	...
	if (obj.attributes.step.value == 1 || (obj.attributes.step.value == 3 && obj.classList.contains('step-back'))) {
		loadingLoad('loading-modal', true);
		datatable.startTable(obj.attributes.idstep.value)
	} else if (obj.attributes.step.value == 2 && !obj.classList.contains('step-back')) {
		let item_sel = datatable.getSelectedItems().length;
		if (item_sel == 0) {
			alertObj.type = 'danger';
			alertObj.msg = 'Debes seleccionar un negocio para ir al siguiente paso!';
			alertLoad(alertObj, 'alertas-modal');
			return;
		}
		//Paso 3
		let id = datatable.getSelectedItems()[0].id
		
		
		loadingLoad('loading-modal', true);
		dtitems.startTable(obj.attributes.idstep.value,id)

	} else if (obj.attributes.step.value == 3 && !obj.classList.contains('step-back')) {
		let negocio = datatable.neg_selected;
		dtitems.renderResumen(obj.attributes.idstep.value,negocio);
	
	}

	if ((obj.attributes.step.value != 1 && obj.classList.contains('step-back')) || (obj.attributes.step.value != 4 && !obj.classList.contains('step-back'))) {
		document.getElementById(`stepdiv${obj.attributes.step.value + obj.attributes.idstep.value}`).style.display = 'none'
		document.getElementById(`stepdiv${String(parseInt(obj.attributes.step.value) + (obj.classList.contains('step-back') ? -1 : +1)) + obj.attributes.idstep.value}`).style.display = ''
	}
